...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
	...
	{
		if (key==subroot->elements[idx].key) return subroot->elements[idx].value;
	}
	if (subroot->children[idx]!=NULL) return find(subroot->children[idx], key);
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
	...
	if (idx < subroot->elements.size() && pair.key==subroot->elements[idx].key) return;
	if (!subroot->is_leaf) {
