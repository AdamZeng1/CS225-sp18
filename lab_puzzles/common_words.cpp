...
void CommonWords::init_common()
{
    ...
    for (size_t i = 0; i < file_word_maps.size(); i++) {
		...
		for (iter=file_word_maps[i].begin(); iter!=file_word_maps[i].end(); ++iter)
		{
			map<string, unsigned int>::iterator lookup = common.find(iter->first);
...
vector<string> CommonWords::get_common_words(unsigned int n)
{
    ...
    /* Your code goes here! */
	map<string, unsigned int>::iterator iter;
	for (iter=common.begin(); iter!=common.end(); ++iter)
	{
		if (iter->second == file_word_maps.size())
		{
			...
			{
				if (file_word_maps[i][iter->first] < n)
					break;
			}
