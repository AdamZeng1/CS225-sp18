...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
	...
	if (dict.find(w1) != dict.end())
	{
		...
		std::cout << *(dict.at(w1).begin()) << std::endl;
		if (std::find(dict.at(w1).begin(), dict.at(w1).end(), w2) != dict.at(w1).end() )
		{
			std::cout << *(dict.at(w1).begin()) << std::endl;
			return true;
		}
	...
	if (dict.find(w2) != dict.end())
	{
		...
		if (std::find(dict.at(w2).begin(), dict.at(w2).end(), w1) != dict.at(w2).end() )
		{
			return true;
		}
	}*/
