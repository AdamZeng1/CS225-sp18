                            ...
                            queue<TreeNode*>& mergeQueue)
{
     ...
     */
	TreeNode * node_return;
	if (mergeQueue.size()==0)
	{
		node_return = singleQueue.front();
		singleQueue.pop();
		return node_return;
	}
	if (singleQueue.size()==0)
	{
		node_return = mergeQueue.front();
		mergeQueue.pop();
		return node_return;
	}
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
	...
	for (unsigned i=0; i<frequencies.size(); i++){
		TreeNode* freq = new TreeNode(frequencies[i]);
	...
	while (singleQueue.size()+mergeQueue.size() >=2 )
	{
		TreeNode* node_left;
		TreeNode* node_right;
		node_left = HuffmanTree::removeSmallest(singleQueue, mergeQueue);
		node_right = HuffmanTree::removeSmallest(singleQueue, mergeQueue);
		int freq_value = node_left->freq.getFrequency() + node_right->freq.getFrequency();
		TreeNode* node_sum = new TreeNode(freq_value);
		node_sum->left = node_left;
		node_sum->right = node_right;
		//push bach the sum node
		mergeQueue.push(node_sum);
	}
