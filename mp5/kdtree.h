...
class KDTree
{
  ...
  private:
	...
	int partition(vector<Point<Dim>> & Points, int left, int right, int pivotIndex, int d);
	void select(vector<Point<Dim>> & Points, int left, int right, int k, int d);
	KDTreeNode * createtree(vector<Point<Dim>>& Points, int left, int right, int d);
	void clear();
	KDTreeNode * copy(const KDTreeNode* subRoot);
	void clear(KDTreeNode* subRoot);

	// helper functions for findnearestneighbor
	// forward traversal helper function
	...
	// check whether the child of the subRoot node needs to be checked
	bool hypersphere_check(const Point<Dim>& query, const Point<Dim>& target, 
							int d, double & Dist) const;
	// backward traversal helper function
